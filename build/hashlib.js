...
var HASH_ALGORITHM, _validateData, crypto, hashingFunction, stringifier;
...
_validateData = function(data) {
  ...
  if (!(data.is_label || !data.hasOwnProperty('is_label'))) {
    throw new Error('is_label must be present with true value or not present');
  }
  if (!(Object.keys(data).length === 4 || Object.keys(data).length === 3 && !data.hasOwnProperty('is_label'))) {
    throw new Error('there should exactly be the attributes camera, furniture_type, structure and optionally is_label');
  }
};

hashingFunction = function(data, hmacKey) {
  var hash, stringToHash, toReturn;
  _validateData(data);
  stringToHash = stringifier.stringify(data);
  if (hmacKey) {
    return crypto.createHmac(HASH_ALGORITHM, hmacKey).update(stringToHash).digest('hex');
  }
...
},{"./stringify":12,"crypto":5}],12:[function(require,module,exports){
var StableJSONStringify, _,
...
StableJSONStringify = (function() {
  ...
  StableJSONStringify.prototype._init = function() {
    ...
    this.cmp = this.opts.cmp && (function(f) {
  ...
  StableJSONStringify.prototype._stringify = function(parent, key, node, level) {
    ...
    if (this._isArray(node)) {
      ...
      keys = _.keys(node).sort(this.cmp && this.cmp(node));
      out = [];
